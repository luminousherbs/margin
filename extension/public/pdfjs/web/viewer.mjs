...
class CaretBrowsingMode {
  ...
  static #caretPositionFromPoint(x, y) {
    if (!document.caretPositionFromPoint) {
      const { startContainer: offsetNode, startOffset: offset } = document.caretRangeFromPoint(
        x,
        y
      );
      return {
...
class CommentSidebar extends Sidebar {
  ...
  #createCommentElement(annotation) {
    const { id, creationDate, modificationDate, richText, contentsObj, color, opacity } =
      annotation;
    const commentItem = document.createElement('li');
  ...
  async open(uiManager, editor, posX, posY, options) {
    ...
    if (editor.hasDefaultPopupPosition()) {
      ...
      if (parentDimensions) {
        if (
          this.#isLTR &&
          posX + dialogWidth > Math.min(parentDimensions.x + parentDimensions.width, innerWidth)
        ) {
...
class CommentPopup {
  ...
  toggle(editor, isSelected, visibility = undefined, isEditable = true) {
    ...
    const { contentsObj, richText, creationDate, modificationDate, color, opacity } =
      editor.getData();
  ...
  #setPosition(x, y, correctPosition) {
    if (!correctPosition) {
      ...
      if (this.#isLTR) {
        ...
        x = Math.min(
          x,
          (window.innerWidth - parentRect.x) / parentRect.width - widthRatio - margin
        );
      }
    }
    this.#posX = x;
    ...
    this.#menu.addEventListener(
      ...
      (e) => {
        switch (e.key) {
          ...
          case 'End':
            this.#menuItems
              .findLast((item) => !item.disabled && !item.classList.contains('hidden'))
              .focus();
            stopEvent(e);
            break;
          default:
...
class PDFThumbnailViewer {
  ...
  #onStopDragging(isDropping = false) {
    ...
    if (
      ...
      !(
        ...
        (selectedPages.has(lastDraggedOverIndex + 1) || selectedPages.has(lastDraggedOverIndex + 2))
      )
    ) {
      ...
      const newCurrentPageId = pagesMapper.getPageId(
        isNaN(this.#pageNumberToRemove) ? pagesToMove[0] : this.#pageNumberToRemove
      );
  ...
  #findClosestThumbnail(x, y) {
    ...
    xArray =
      yPos === positionsY.length - 1 && positionsLastX.length > 0 ? positionsLastX : positionsX;
...
class PDFPageView extends BasePDFPageView {
  ...
  async draw() {
    ...
    const recordBBoxes =
      this.enableOptimizedPartialRendering && this.#hasRestrictedScaling && !this.recordedBBoxes;
    const transform = outputScale.scaled ? [outputScale.sx, 0, 0, outputScale.sy, 0, 0] : null;
...
function isValidAnnotationEditorMode(mode) {
  return (
    Object.values(AnnotationEditorType).includes(mode) && mode !== AnnotationEditorType.DISABLE
  );
}
  ...
  scrollPageIntoView({
    ...
    if (center) {
      if (center === 'both' || center === 'vertical') {
        top -=
          (this.container.clientHeight - Math.abs(boundingRect[1][1] - boundingRect[0][1])) / 2;
      }
      if (center === 'both' || center === 'horizontal') {
        left -=
          (this.container.clientWidth - Math.abs(boundingRect[1][0] - boundingRect[0][0])) / 2;
      }
...
class ViewsManager extends Sidebar {
  ...
  constructor({
    ...
    this.eventBus = eventBus;
    this.menu = new Menu(viewsManagerSelectorOptions, viewsManagerSelectorButton, [
      thumbnailButton,
      outlineButton,
      attachmentsButton,
      layersButton,
...
const PDFViewerApplication = {
      ...
      AppOptions.get('enableComment') && appConfig.editCommentDialog
        ? new CommentManager(
            appConfig.editCommentDialog,
            {
              ...
              sidebarResizer:
                appConfig.annotationEditorParams?.editorCommentsSidebarResizer || null,
              ...
              closeButton:
                appConfig.annotationEditorParams?.editorCommentsSidebarCloseButton || null,
  ...
  async _initializeMetadata(pdfDocument) {
    const { info, metadata, contentDispositionFilename, contentLength, hasStructTree } =
      await pdfDocument.getMetadata();
    if (pdfDocument !== this.pdfDocument) {
      return;
    }
  ...
  async onSavePagesEditedPDF({ data: { includePages, excludePages, pageIndices } }) {
    if (!this.pdfDocument) {
      return;
    }
    const pageInfo = {
